#!/usr/bin/env python
"""
Seed MySQL `market_data` table with sample data.

Run after creating tables:
  .\venv\Scripts\Activate
  pip install -r requirements.txt
  python .\scripts\seed_mysql.py
"""
import asyncio
import uuid
from datetime import datetime
import sys
import os
from urllib.parse import urlparse, unquote

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from app.core.config import settings

try:
    import aiomysql
except Exception:
    print("aiomysql not installed. Run: pip install aiomysql")
    raise


ROWS = [
    ("AAPL",  "Apple Inc.",              189.30,   1.25,  55000000,  2950000000000),
    ("MSFT",  "Microsoft Corporation",   415.20,   0.54,  18000000,  3080000000000),
    ("GOOGL", "Alphabet Inc.",           175.50,   0.87,  22000000,  2180000000000),
    ("AMZN",  "Amazon.com Inc.",         185.60,   1.10,  32000000,  1920000000000),
    ("META",  "Meta Platforms Inc.",     505.40,   2.10,  15000000,  1290000000000),
    ("NVDA",  "NVIDIA Corporation",      875.90,   3.40,  42000000,  2160000000000),
    ("TSLA",  "Tesla Inc.",              245.80,  -2.30,  85000000,   782000000000),
    ("NFLX",  "Netflix Inc.",            610.50,   0.75,   9000000,   267000000000),
    ("AMD",   "Advanced Micro Devices",  165.40,   1.90,  35000000,   267000000000),
    ("INTC",  "Intel Corporation",        35.20,  -0.60,  28000000,   148000000000),
    ("BTC",   "Bitcoin",              68500.00,   1.80,    500000,  1340000000000),
    ("ETH",   "Ethereum",              3500.00,   2.20,   1200000,   420000000000),
    ("BNB",   "BNB",                    580.00,   0.90,    800000,    86000000000),
    ("SOL",   "Solana",                 175.00,   3.50,   2000000,    81000000000),
    ("XRP",   "XRP",                      0.58,  -0.40,  45000000,    32000000000),
]


async def main():
    url = urlparse(settings.DATABASE_URL)
    user = unquote(url.username) if url.username else 'root'
    password = unquote(url.password) if url.password else ''
    host = url.hostname or 'localhost'
    port = url.port or 3306
    dbname = url.path.lstrip('/') or 'marketbuddy'

    print(f"Connecting to MySQL {host}:{port} as {user}")
    conn = await aiomysql.connect(host=host, port=port, user=user, password=password, autocommit=True)
    async with conn:
        cur = await conn.cursor()
        try:
            print(f"Using database '{dbname}'")
            await cur.execute(f"CREATE DATABASE IF NOT EXISTS `{dbname}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;")
            await cur.execute(f"USE `{dbname}`;")

            # Ensure unique index on symbol exists
            try:
                await cur.execute("ALTER TABLE market_data ADD UNIQUE INDEX ux_market_symbol (symbol);")
            except Exception:
                # ignore if index exists or table missing
                pass

            # Insert rows with ON DUPLICATE KEY UPDATE
            insert_sql = (
                "INSERT INTO market_data (id, symbol, name, price, change_percent, volume, market_cap, last_updated) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) "
                "ON DUPLICATE KEY UPDATE name=VALUES(name), price=VALUES(price), change_percent=VALUES(change_percent), volume=VALUES(volume), market_cap=VALUES(market_cap), last_updated=VALUES(last_updated);"
            )

            now = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
            for sym, name, price, change, volume, market_cap in ROWS:
                row_id = str(uuid.uuid4())
                params = (row_id, sym, name, float(price), float(change), int(volume), float(market_cap), now)
                await cur.execute(insert_sql, params)

            print("Seeding complete.")
        finally:
            await cur.close()


if __name__ == '__main__':
    asyncio.run(main())
